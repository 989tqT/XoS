...
def verify_no_symlinks_in_ancestors(target_path: Path, root_path: Path) -> None:
    ...
    while curr != resolved_root and curr != curr.parent:
        try:
            ...
            # 2. check for windows reparse point / junction point
...
def sanitize_and_resolve_path(
    ...
    *,
    ...
    Args:
        ...
        session_id: Optional session UUID to dynamically allow scratchpad writes.
    ...
    """
    # 1. block absolute path, drive, and anchor (include platform-cross detection)
    ...
    # 2. block windows reserved device name
    ...
    # resolve and verify session scratchpad dynamically if session_id are provided
    roots_to_check = list(allowed_roots)
    if session_id is not None:
        from xos.core.state import get_db_path, verify_session

        try:
            scratchpad_path = verify_session(get_db_path(), session_id)
            roots_to_check.append(scratchpad_path)
        except Exception as exc:
            raise ValueError(f"Access denied: invalid or expired session: {exc}") from exc

    validated_path: Path | None = None
    matching_root: Path | None = None

    # 3. resolve and verify boundary against each allowed root
    for root in roots_to_check:
        try:
            ...
            if write_mode:
                # verify that no ancestor in requested path is symlink or junction point
    ...
    if validated_path is None or matching_root is None:
        # check if combined path exist under any root but were rejected
        for root in roots_to_check:
            try:
                resolved_root = root.resolve(strict=True)
    ...
    # perform integrity check after resolve to absolute canonical path
    ...
    # 5. write-specific hardening control
    if write_mode:
        # prohibit symlink on target file itself if it already exist
        try:
    ...
    # 6. enforce that target must be file (not directory or special device) if it exist
