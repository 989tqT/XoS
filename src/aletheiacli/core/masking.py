...
# Bounded Regex Patterns to prevent ReDoS (Zero-Dependency approach with rigorous pattern hardening)
...
MASK_PATTERNS = [
    ...
    # Email addresses (Strictly bounded to avoid ReDoS)
    (
...
def mask_line(line: str) -> str:
    ...
    """
    # 1. Defense-in-depth: Truncate long lines to prevent CPU/memory exhaustion and ReDoS
...
def secure_envelope_cdata(content: str) -> str:
    ...
    # 1. Mask multiline patterns (like SSH Private Keys) globally first
    for pattern, replacement in MULTILINE_MASK_PATTERNS:
        content = pattern.sub(replacement, content)

    # 2. Process line-by-line to prevent ReDoS on massive assembled blocks
